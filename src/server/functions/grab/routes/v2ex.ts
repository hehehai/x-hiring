...
export async function v2exListRoute(page: string) {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`列表页面请求失败: ${res.status} ${res.statusText}`);
    }
    ...
    console.error("v2exListRoute error", err);
    return [];
  }
}
...
export async function v2exDetailRoute(detailId: string) {
  try {
    ...
    if (!res.ok) {
      throw new Error(`详情页面请求失败: ${res.status} ${res.statusText}`);
    }
    const text = await res.text();
    if (!text) {
    ...
    console.error("v2exDetailRoute error", err);
    return null;
  }
}
