...
// 字符串数组去重
export const uniqueStringArray = (str: string): string[] => {
	if (!str) {
		return [];
	}
	return uniqueArray(stringToArray(str));
};

// 有效日期
export const convertToValidDate = (
...
// 有效日期范围
export const convertToValidDateRange = (
	dateRange: (Date | null | undefined)[],
): (Date | undefined)[] => {
	return dateRange.map((date) => convertToValidDate(date));
};
